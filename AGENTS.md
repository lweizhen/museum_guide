...
LoRA 样本应优先对齐 `vl_rag_lora` 链路：除 `identify` 外，caption、guide style、QA 样本都应包含 `grounding_context`，instruction 中要显式给出参考资料，训练目标是“图像 + RAG 上下文 + 导览式回答”。导览增强样本应包含 overview、highlight、story 三类讲解任务，用于提升讲解风格、完整性和吸引力。
