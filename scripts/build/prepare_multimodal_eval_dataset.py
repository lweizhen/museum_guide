...
def _guide_overview_output(fields: dict[str, str]) -> str:
    ...
    if value:
        if value.startswith(("反映", "体现", "展现", "折射", "说明", "见证", "对研究", "为研究")):
            parts.append(f"从这件文物上，我们也能看到{_value_to_seen_clause(value)}")
        else:
            parts.append(f"从这件文物上，我们也能看到{value}")
...
def _build_llm_target_prompt(
    ...
    return (
        ...
        "3. 不要使用‘从导览的角度’‘从研究价值来看’‘如果从用途来看’‘通过它，我们可以了解到’‘今天再看它’这类套话或元话语。\n"
        "4. 不要写成资料摘抄，不要机械罗列字段，不要照抄模板参考答案的句式。\n"
        "5. 不要过度抒情，不要拟人化，不要写‘仿佛’‘静静地躺在展柜里’‘像一位少女’这类文学化表达。\n"
        "6. 可以使用自然导览表达，如‘大家现在看到的是……’‘先看这里……’‘把它放回当时看……’，但整体要简洁稳妥。\n"
        "7. 避免空泛拔高，例如‘不可或缺’‘极具代表性’‘重要参考价值’，除非资料本身已经明确支持。\n"
        "8. 如果是问答型任务，先直接回答问题，再用一两句自然补充；如果是讲解型任务，写成完整连贯的一小段讲解。\n"
        "9. 不要输出标题、标签、字段名、括号说明、分点或额外解释，只输出最终回答。\n\n"
        f"文物名称：{name}\n"
        f"任务类型：{task}\n"
        f"任务要求：{task_hint}\n"
        f"用户指令：{instruction}\n"
